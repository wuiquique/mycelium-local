...
public interface CartRepo extends GenericRepository<Cart, Integer> {
    ...
    @Query("SELECT c.*, p_.\"name\" AS p_name, p_.\"desc\" AS p_desc, p_.\"weight\" AS p_weight, p_.\"categorieId\" AS p_categorieId, p_.\"brand\" AS p_brand, p_.\"quantity\" AS p_quantity, p_.\"price\" AS p_price, j_.\"url\" AS j_url FROM \"carts\" c INNER JOIN \"products\" p_ ON c.\"productId\" = p_.\"id\" LEFT JOIN \"pictures\" j_ ON j_.\"productId\" = p_.\"id\" WHERE \"userId\" = :userId ")
